...
public:
  void setParametros(int tamPob, double probCruce, double probMut, int blEvalPorInd) {
    ...
    this->blEvaluaciones = blEvalPorInd;
  }
  ...
  ResultMH optimize(Problem *problem, int maxevals) override {
    ...
    while (evaluaciones < maxevals) {
      ...
      gen++;
      if (gen % 10 == 0) {
        // Índices de los mejores 10%
        vector<int> indices(tamPoblacion);
        iota(indices.begin(), indices.end(), 0);
        sort(indices.begin(), indices.end(),
             [&](int a, int b) { return fitnesses[a] > fitnesses[b]; });

        int numBL = max(1, (int)(0.1 * tamPoblacion));  // al menos 1
        for (int i = 0; i < numBL && evaluaciones < maxevals; ++i) {
          int idx = indices[i];
          if (evaluaciones + blEvaluaciones > maxevals) break;
          ResultMH mejorado = bl.optimize(problem, poblacion[idx], fitnesses[idx], blEvaluaciones);
          poblacion[idx] = mejorado.solution;
          fitnesses[idx] = mejorado.fitness;
