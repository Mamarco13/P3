...
class AM_10_01 : public MH {
public:
  void setParametros(int tamPob, double probCruce, double probMut, int blEvalPorInd, double probAplicacionBL) {
    this->tamPoblacion = tamPob;
    this->probCruce = probCruce;
    this->probMutacion = probMut;
    this->blEvaluaciones = blEvalPorInd;
    this->probAplicarBL = probAplicacionBL;
  }
  ...
  ResultMH optimize(Problem *problem, int maxevals) override {
    ...
    while (evaluaciones < maxevals) {
      ...
      gen++;
      if (gen % 10 == 0) {
        for (int i = 0; i < tamPoblacion && evaluaciones < maxevals; ++i) {
          if (Random::get(0.0, 1.0) < probAplicarBL && evaluaciones + blEvaluaciones <= maxevals) {
